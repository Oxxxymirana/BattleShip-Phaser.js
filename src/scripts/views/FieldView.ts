...
export class FieldView extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene: Phaser.Scene, model: Field, secondBoardX: any, atlas: string) {
        super(scene, 0, 0, atlas, 'closed');
        this.atlas = atlas;
        this._model = model;
    ...
    private get _offset(): Vec2 {
        if(this.secondBoardX) {
            
            return {
                x: (this.scene.cameras.main.width - this._model.board.cols * this.width) / 1.5,
                y: (this.scene.cameras.main.height - this._model.board.rows * this.height) / 2,
            };

        }
        return {
            x: (this.scene.cameras.main.width - this._model.board.cols * this.width) / 4,
            y: (this.scene.cameras.main.height - this._model.board.rows * this.height) / 2,
        };
    }
